...
> **Promotion only.** Use this after lessons are already approved for durable
> memory promotion. Use `session-lessons` to discover recurring session
> patterns, and use `pr-retrospective` for one merged PR's learnings.

## When to use

- When promoting mature lessons from a domain memory file into its instruction file
- After reviewing and approving session learnings for permanent retention
- When the user says "merge this into memory" or invokes `/memory-merger`
